...
#This script is the wrapper to run 1Mb conventional (PrediXcan) expression modeling
...
do
	...
	qsub -d ${out_dir} -N ${tiss}_pr_chr${chrom} -v chrom="${chrom}",prefix="${prefix}",out_dir="${out_dir}",cis_window="${cis_window}",alpha="${alpha}",n_folds="${n_folds}",n_train_test_folds="${n_train_test_folds}",expression_RDS="${expression_RDS}",geno_file="${geno_file}",gene_annot_RDS="${gene_annot_RDS}",snp_annot_RDS="${snp_annot_RDS}",covariates_file="${covariates_file}" ./scripts/run_nested_cvenet_function_LIBD.pbs 
