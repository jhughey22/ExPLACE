...
adjust_for_covariates <- function(expression_vec, cov_df) {
  ...
  expr_resid_r1 <- scale(expr_resid_r1, center = TRUE, scale = TRUE)
  #expr_resid_r1 <- data.frame('expression_vec' = qqnorm(expr_resid_r1, plot.it = F)$x)
  #expr_resid_r1 <- data.frame('expression_vec' = expr_resid_r1)
  ...
  #expr_resid_libd_df <- scale(expr_resid_libd, center = TRUE, scale = TRUE)
  #expr_resid_libd_df <- data.frame('expression_vec' = qqnorm(expr_resid_libd, plot.it = F)$x)
  expr_resid_libd_df <- data.frame('expression_vec' = expr_resid_libd)
  ...
  combined_df_full <- merge(cov_df, expression_vec_combined_df_final, by = 0, sort = F)
...
nested_cv_elastic_net_model_comp <- function(cis_gt, cis_gt_window, y, n_samples, n_train_test_folds, n_k_folds, alpha, 
  ...
  #If keeping folds same across models
...
main <- function(snp_annot_RDS, gene_annot_RDS, geno_file, expression_RDS, 
  ...
  # #test to add Study and smoking
  # covariates_df$Study <- 0
  # #View(covariates_df)
  # covariates_df[58:115, 'Study'] <- 1
  # covariates_df$Smoking.status.n <- 0
  # covariates_df[87:115, 'Smoking.status.n'] <- 1
  ...
  for (i in 1:n_genes) {
    ...
    if (ncol(cis_gt_window) >= 2) {
      ...
      # covariates_df$Smoking.status.n <- 0
      # covariates_df[87:115, 'Smoking.status.n'] <- 1
      #now adjust for covariates
      #this is adapted for LIBD set as a two part adjustment
      if(!is.na(covariates_file)) {
      ...
      if (length(fit) > 0) {
        ...
        #check if there are any non-zero weight snps (any significant snps)       
        if (fit$df > 0) {
...
# seed <- 1993
# out_dir <- '/gpfs/group/dxl46/default/private/jordan/ABC_TWAS/joblogs/weighted_hybrid_Enet_joblogs/NA_newABC_1mb_r2'
